...
| omp runner | `adapters/omp/runner.sh` (+ `conformance-driver.sh`) | L2 bus-integrated wrapper for omp; spawned by orchestrator with pre-claimed packet; conformance-gated via the `--adapter` driver |
...
| omp runner (L2 bus-integrated wrapper; `adapters/omp/runner.sh`; 18/18 smoke) | **live + conformance-gated** — `--claimed` pre-claimed packet contract, `AGENTS_TASK_ID` export, neutral `task_progress` milestones, end-of-run work-state milestone via shared `adapters/lib/checkpoint.sh`, `COMPLETE_CMD pass/fail`, policy scan; `OMP_BIN`/`PROGRESS_CMD`/`COMPLETE_CMD`/`CHECKPOINT_CMD` seams. Wired into the adapter contract via `adapters/omp/conformance-driver.sh` (C13 adapter mode, 28/0/0); advertises `turn-end`+`session-end` (one-shot processes terminate, unlike OpenCode). Smoke T6 exercises the real `bin/claim`/`bin/progress`/`bin/complete` + checkpoint end-to-end | 2026-06-20 |
